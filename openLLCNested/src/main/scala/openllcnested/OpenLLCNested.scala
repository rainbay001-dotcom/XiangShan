...
// Configuration key for OpenLLCNested parameters
...
//     DAT  ↔      (bidirectional data)
//
// SNP channel on io.sn: OpenLLC must expose SNP on its io.rn port when NESTED is
// connected as an RN, enabling OpenLLC to send coherence probes to NESTED's cached state.
class OpenLLCNested()(implicit p: Parameters) extends Module {

  val nestedParams = p(OpenLLCNestedParamKey)
  val numRNs       = nestedParams.clientCaches.length
  // Derive node-ID width from CHI issue without depending on system.HasSoCParameter.
  // This mirrors SoCParameters.NodeIDWidthList defined in the main XiangShan project.
  val nodeIDWidth = p(CHIIssue) match {
    case "B"   => 7
    case "C"   => 7
    case "E.b" => 11
    case _     => 11  // default to widest supported width
  }

  val io = IO(new Bundle {
    // HN-facing ports: accept CHI REQ from Core L2 RNs
    val rn = Vec(numRNs, Flipped(new PortIO))
    // RN-facing port: issue CHI REQ to OpenLLC HN; receives SNP from OpenLLC
    val sn = new PortIO
    val nodeID = Input(UInt(nodeIDWidth.W))
    // debugTopDown: Top-Down microarchitecture analysis sideband.
    // Uses fullAddressBits from OpenLLCParam instead of soc.PAddrBits so that this
    // standalone module does not depend on system.HasSoCParameter.
    val debugTopDown = new Bundle {
      val robHeadPaddr = Input(Vec(numRNs, Valid(UInt(nestedParams.fullAddressBits.W))))
  ...
  //   a. Receive SNP from OpenLLC on io.sn (requires SNP channel on OpenLLC rn port)
  ...
  // Placeholder wiring — replace with actual cache coherence logic
