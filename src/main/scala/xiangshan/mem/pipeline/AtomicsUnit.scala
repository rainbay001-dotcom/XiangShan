...
class AtomicsUnit(val param: ExeUnitParams)(implicit p: Parameters) extends XSModule
  ...
  // PMA-NC: non-cacheable main memory (pmp.mmio=true but pmp.atomic=true)
  // Distinguished from real MMIO (pmp.mmio=true and pmp.atomic=false) using pmp.atomic bit.
  val nc_via_pma_reg = RegInit(false.B)
  // Local LR/SC reservation for NC memory (no cache-line coherence available for NC)
  val lr_valid = RegInit(false.B)
  val lr_addr  = Reg(UInt(PAddrBits.W))

  when (state === s_pm) {
    val pmp = WireInit(io.pmpResp)
    // PMA-NC: mmio=true (non-cacheable) but atomic=true (supports atomic ops)
    ...
    // Exception for: explicit IO ordering (Svpbmt IO) or PMA-MMIO without atomic support
    // NC memory (Svpbmt NC or PMA-NC with atomic=true) is allowed to do AMO → no exception
    val exception_pa_mmio = Pbmt.isIO(pbmtReg) ||
      (Pbmt.isPMA(pbmtReg) && pmp.mmio && !pmp.atomic)
    ...
    when (exception_va || exception_pa || nc_casq_fault) {
      state := s_finish
      out_valid := true.B
      atom_override_xtval := true.B
      when (nc_casq_fault) {
      ...
      // NC AMO → uncache path; cached AMO → dcache path
      state := Mux(is_nc,
        Mux(sbuffer_empty, s_nc_req, s_wait_flush_sbuffer_resp),
  ...
  when (state === s_wait_flush_sbuffer_resp) {
    when (sbuffer_empty) {
      val is_nc = is_nc_reg || nc_via_pma_reg
      state := Mux(is_nc, s_nc_req, s_cache_req)
    }
  }
  ...
  when (state === s_nc_resp) {
    when (io.uncache.resp.valid) {
      ...
      // SC: MMIOBridge encodes Exclusive Monitor result in bit[0]:
      //   0 = ExOkay (SC success → rd=0); 1 = OK (SC fail → rd=1)
      // LR/AMO/AMOCAS: sign-extend the original memory value returned by HN-F
      resp_data := Mux(isSc,
        nc_data(0),   // SC result: 0=success(rd=0), 1=fail(rd=1)
        Mux(lsuSize === LSUOpType.D.U,
          SignExt(nc_data(63, 0), QuadWordBits),
          SignExt(rdataSel(31, 0), QuadWordBits)  // W or AMOCAS.W
        )
      )
      when (isSc) { success := nc_data(0) === 0.U }  // ExOkay=success
  ...
  // NC AMO instructions skip DiffTest register comparison (NEMU and DUT NC memory state may diverge)
  io.out.debug.isNCIO := is_nc_reg || nc_via_pma_reg
  ...
  // NC AMO path: drive io.uncache.req to UncacheAtomicBuffer
  // SC with no valid reservation is handled as early fail in s_nc_req itself
