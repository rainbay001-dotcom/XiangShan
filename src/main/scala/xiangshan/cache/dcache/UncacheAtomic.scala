...
import xscache.coupledL2.{MemBackTypeMMField, MemPageTypeNC, MemPageTypeNCField}
import xiangshan._
import utils._
import utility._
    ...
    with MemoryOpConstants
{
  ...
  val loaded = Reg(UInt(XLEN.W))  // original value read from memory
  ...
  //--------------------------------------------------------------------------
  // AMOCAS compare (evaluated while waiting for s_resp bus.d)
  //--------------------------------------------------------------------------
  ...
  //--------------------------------------------------------------------------
  io.resp.valid :=
    (state === s_resp    && bus.d.valid && (isLr || isSc || isRegAMO)) ||
    (state === s_resp    && bus.d.valid && isAMOCAS && !cas_match)      ||
    (state === s_cas_ack && bus.d.valid)
