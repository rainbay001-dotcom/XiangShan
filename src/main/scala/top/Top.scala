    ...
    with HasDTSImp[XSTop]
  {
    ...
    withClockAndReset(io.clock, io.reset) {
      Option.when(enableCHI)(true.B).foreach { _ =>
        ...
        //   L2 (RN) → CHIHNSubNode (HN to L2 / RN to OpenLLC) → OpenLLC (global HN)
        val chiHNSubNode = Module(new CHIHNSubNode(core_with_l2.length))

        for ((core, i) <- core_with_l2.zipWithIndex) {
          val mmioLogger = CHILogger(s"L2[${i}]_MMIO", true)
          // Logger between L2 (RN) and HN sub-node (presents as HN to L2)
          val hnLogger   = CHILogger(s"L2[${i}]_HN", true)
          // Logger between HN sub-node (presents as RN to OpenLLC) and OpenLLC
          ...
          // L2 (RN) → HN sub-node: sub-node acts as HN to L2
          chiHNSubNode.io.rnFromL2(i) <> hnLogger.io.down
          // HN sub-node (RN) → OpenLLC: sub-node acts as RN to OpenLLC (global HN)
